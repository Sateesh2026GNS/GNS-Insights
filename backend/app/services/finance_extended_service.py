...
from app.models.accounts import Expense, Income, JournalEntry, JournalLeg, GLAccount, FixedAsset
from app.models.inventory import Supplier, StockLevel, InventoryItem
from app.models.procurement import PurchaseOrder, SupplierPayment, VendorBill
from app.models.sales import Customer, Invoice, Payment, InvoiceItem
from app.schemas.finance_extended import (
    APListRead,
    APSummaryRead,
    ARListRead,
...
def get_ap_summary(db: Session, tenant_id: int) -> APSummaryRead:
    ...
    # If no vendor bills exist, derive outstanding from approved/received POs
    if not bills:
        pos = list(db.scalars(
            select(PurchaseOrder).where(
                PurchaseOrder.tenant_id == tenant_id,
                PurchaseOrder.status.in_(("approved", "received", "partial", "pending")),
            )
        ).all())
        outstanding = sum(float(po.total_amount or 0) for po in pos)
        overdue = sum(1 for po in pos if po.expected_date and po.expected_date < today)
        due_week = sum(1 for po in pos if po.expected_date and today <= po.expected_date <= week_end)
        paid_month = float(
            db.scalar(
                select(func.coalesce(func.sum(SupplierPayment.amount), 0)).where(
                    SupplierPayment.tenant_id == tenant_id,
        ...
        return APSummaryRead(
            ...
            overdue_bills=overdue,
            paid_this_month=paid_month,
            pending_approvals=sum(1 for po in pos if po.status == "pending"),
            vendor_count=vendors,
        )
...
def list_ap_enriched(db: Session, tenant_id: int) -> list[APListRead]:
    ...
    # No vendor bills — fall back to Purchase Orders so the table is not empty
    if not bills:
        pos = list(
            db.scalars(
        ...
        for po in pos:
            ...
            paid = amt if status in ("paid", "closed") else 0.0
            balance = 0.0 if status in ("paid", "closed") else amt
            result.append(
                APListRead(
                    id=po.id,
                    ...
                    due_date=po.expected_date.isoformat() if po.expected_date else None,
                    amount=amt,
                    gst=amt * 0.18,
                    paid=paid,
                    balance=balance,
...
def get_gl_summary(db: Session, tenant_id: int) -> GLSummaryRead:
    ...
    receivables = float(
        db.scalar(
            select(func.coalesce(func.sum(Invoice.grand_total), 0) - func.coalesce(func.sum(Invoice.amount_paid), 0)).where(
                Invoice.tenant_id == tenant_id,
                Invoice.status.notin_(("cancelled", "void")),
            )
        ) or 0
    )
    assets = max(receivables, 0) + max(cash_balance, 0) + revenue * 0.3
    liabilities = po_outstanding + exp * 0.2
    equity = assets - liabilities
    return GLSummaryRead(
...
def list_gl_enriched(db: Session, tenant_id: int) -> list[GLListRead]:
    ...
    invs = list(db.scalars(
        select(Invoice).where(Invoice.tenant_id == tenant_id, Invoice.status.notin_(("cancelled", "void")))
        .order_by(Invoice.issue_date.desc()).limit(30)
    ).all())
    for inv in invs:
        amt = float(inv.grand_total or 0)
        yr = inv.issue_date.year if inv.issue_date else date.today().year
        entries.append(GLListRead(
            id=seq, voucher_no=f"JV-{yr}-{seq:04d}",
            entry_date=inv.issue_date.isoformat() if inv.issue_date else None,
            account="Accounts Receivable", debit=amt, credit=0, balance=amt,
            narration=f"Sales invoice {inv.invoice_number}", cost_center="Sales", branch="Head Office",
        ))
        seq += 1
        entries.append(GLListRead(
            id=seq, voucher_no=f"JV-{yr}-{seq:04d}",
            entry_date=inv.issue_date.isoformat() if inv.issue_date else None,
            account="Sales Revenue", debit=0, credit=amt, balance=amt,
            narration=f"Sales invoice {inv.invoice_number}", cost_center="Sales", branch="Head Office",
        ))
        seq += 1

    exps = list(db.scalars(
        select(Expense).where(Expense.tenant_id == tenant_id)
        .order_by(Expense.expense_date.desc()).limit(20)
    ).all())
    for e in exps:
        amt = float(e.amount or 0)
        yr = e.expense_date.year if e.expense_date else date.today().year
        entries.append(GLListRead(
            id=seq, voucher_no=f"PV-{yr}-{seq:04d}",
            entry_date=e.expense_date.isoformat() if e.expense_date else None,
            account=e.category or "Operating Expense", debit=amt, credit=0, balance=amt,
            narration=e.description or e.vendor or "Expense entry", cost_center=e.category or "Administration", branch="Plant-1",
        ))
        seq += 1

    incomes = list(db.scalars(
        select(Income).where(Income.tenant_id == tenant_id)
        .order_by(Income.income_date.desc()).limit(20)
    ).all())
    for inc in incomes:
        amt = float(inc.amount or 0)
        yr = inc.income_date.year if inc.income_date else date.today().year
        entries.append(GLListRead(
            id=seq, voucher_no=f"RV-{yr}-{seq:04d}",
            entry_date=inc.income_date.isoformat() if inc.income_date else None,
            account=inc.category or "Other Income", debit=0, credit=amt, balance=amt,
            narration=inc.description or inc.category or "Income entry", cost_center="Finance", branch="Head Office",
        ))
        seq += 1

    spays = list(db.scalars(
        select(SupplierPayment).where(SupplierPayment.tenant_id == tenant_id)
        .order_by(SupplierPayment.payment_date.desc()).limit(20)
    ).all())
    for sp in spays:
        amt = float(sp.amount or 0)
        yr = sp.payment_date.year if sp.payment_date else date.today().year
        entries.append(GLListRead(
            id=seq, voucher_no=f"VP-{yr}-{seq:04d}",
            entry_date=sp.payment_date.isoformat() if sp.payment_date else None,
            account="Accounts Payable", debit=amt, credit=0, balance=amt,
            narration=f"Supplier payment ref {sp.reference or sp.id}", cost_center="Procurement", branch="Plant-1",
        ))
        seq += 1

    entries.sort(key=lambda x: x.entry_date or "", reverse=True)
    return entries[:60]


def get_gst_extended(
    db: Session,
    tenant_id: int,
    year: int,
    ...
    # 1. Fetch invoices matching tenant and year
    inv_stmt = select(Invoice).where(Invoice.tenant_id == tenant_id).where(func.extract("year", Invoice.issue_date) == year).where(Invoice.status != "draft")
    ...
    for m in months_list:
        monthly_collection[m] = {"sgst": 0.0, "cgst": 0.0, "igst": 0.0, "total": 0.0}
        
    for i in invs:
        if i.issue_date:
            m_name = months_list[i.issue_date.month - 1]
            sg = float(i.sgst_amount or 0)
            cg = float(i.cgst_amount or 0)
            ig = float(i.igst_amount or 0)
            tot = sg + cg + ig
            monthly_collection[m_name]["sgst"] += sg
            monthly_collection[m_name]["cgst"] += cg
            monthly_collection[m_name]["igst"] += ig
            monthly_collection[m_name]["total"] += tot
        
    monthly = [{"month": m, "amount": monthly_collection[m]["total"]} for m in months_list]
    trend = [{"month": m, "sgst": monthly_collection[m]["sgst"], "cgst": monthly_collection[m]["cgst"], "igst": monthly_collection[m]["igst"]} for m in months_list]

    # 2. GST by Customer
    cust_stmt = (
        select(Customer.name, func.sum(Invoice.sgst_amount + Invoice.cgst_amount + Invoice.igst_amount))
    ...
    by_cust = []
    for row in db.execute(cust_stmt).all():
        if row[1] and float(row[1]) > 0:
            by_cust.append({"name": row[0] or "Unknown", "gst": float(row[1])})
            
    by_cust.sort(key=lambda x: x["gst"], reverse=True)

    # 3. GST by Product (InvoiceItem description)
    prod_stmt = (
        select(InvoiceItem.item_description, func.sum((InvoiceItem.qty * InvoiceItem.rate) * (Invoice.sgst_pct + Invoice.cgst_pct + Invoice.igst_pct) / 100))
        .join(Invoice, InvoiceItem.invoice_id == Invoice.id)
        .where(Invoice.tenant_id == tenant_id)
        .where(func.extract("year", Invoice.issue_date) == year)
        .where(Invoice.status != "draft")
        .group_by(InvoiceItem.item_description)
    )
    by_prod = []
    for row in db.execute(prod_stmt).all():
        if row[1] and float(row[1]) > 0:
            by_prod.append({"name": row[0] or "Unknown Product", "gst": float(row[1])})
            
    by_prod.sort(key=lambda x: x["gst"], reverse=True)
...
def get_pl_extended(
    ...
    # Fetch invoices, incomes, expenses for the year
    invs = list(db.scalars(
        select(Invoice).where(Invoice.tenant_id == tenant_id).where(func.extract("year", Invoice.issue_date) == year).where(Invoice.status != "draft")
    ).all())
    
    incomes = list(db.scalars(
        select(Income).where(Income.tenant_id == tenant_id).where(func.extract("year", Income.income_date) == year)
    ).all())
    
    exps = list(db.scalars(
        select(Expense).where(Expense.tenant_id == tenant_id).where(func.extract("year", Expense.expense_date) == year)
    ).all())
    
    months = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    months_list = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    monthly_rev_map = {m: 0.0 for m in months_list}
    monthly_exp_map = {m: 0.0 for m in months_list}
    
    for i in invs:
        if i.issue_date:
            monthly_rev_map[months_list[i.issue_date.month - 1]] += float(i.grand_total or 0)
    for inc in incomes:
        if inc.income_date:
            monthly_rev_map[months_list[inc.income_date.month - 1]] += float(inc.amount or 0)
    for e in exps:
        if e.expense_date:
            monthly_exp_map[months_list[e.expense_date.month - 1]] += float(e.amount or 0)
            
    monthly_rev = [{"month": m, "amount": monthly_rev_map[m]} for m in months]
    exp_trend = [{"month": m, "amount": monthly_exp_map[m]} for m in months]
    profit_trend = [{"month": m, "amount": monthly_rev_map[m] - monthly_exp_map[m]} for m in months]
    rev_vs_exp = [{"month": m, "revenue": monthly_rev_map[m], "expense": monthly_exp_map[m]} for m in months]

    # Group dept costs by category
    dept_cost_map = {}
    for e in exps:
        cat = e.category or "Other"
        dept_cost_map[cat] = dept_cost_map.get(cat, 0.0) + float(e.amount or 0)
    dept_cost = [{"name": name, "amount": val} for name, val in dept_cost_map.items()]
    
    # Factory cost
    factory_cost_map = {}
    for e in exps:
        if "factory" in (e.category or "").lower() or "production" in (e.category or "").lower() or "machine" in (e.category or "").lower():
            cat = e.category or "Factory"
            factory_cost_map[cat] = factory_cost_map.get(cat, 0.0) + float(e.amount or 0)
    factory = [{"name": name, "amount": val} for name, val in factory_cost_map.items()]
...
def get_finance_hub(db: Session, tenant_id: int) -> FinanceHubRead:
    ...
    # 1. Fetch payments for customer receipts
    pays = list(db.scalars(
        select(Payment).where(Payment.tenant_id == tenant_id).where(func.extract("year", Payment.payment_date) == current_year)
    ).all())
    
    # 2. Fetch supplier payments for vendor payments
    spays = list(db.scalars(
        select(SupplierPayment).where(SupplierPayment.tenant_id == tenant_id).where(func.extract("year", SupplierPayment.payment_date) == current_year)
    ).all())
    
    # 3. Monthly group
    months_list = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
    monthly_inflow = {m: 0.0 for m in months_list}
    monthly_outflow = {m: 0.0 for m in months_list}
    
    for p in pays:
        if p.payment_date:
            m_name = months_list[p.payment_date.month - 1]
            monthly_inflow[m_name] += float(p.amount or 0)
            
    for sp in spays:
        if sp.payment_date:
            m_name = months_list[sp.payment_date.month - 1]
            monthly_outflow[m_name] += float(sp.amount or 0)
            
    cash_flow_trend = [{"month": m, "inflow": monthly_inflow[m], "outflow": monthly_outflow[m]} for m in months_list]
    vendor_payments = [{"month": m, "amount": monthly_outflow[m]} for m in months_list]
    customer_receipts = [{"month": m, "amount": monthly_inflow[m]} for m in months_list]

    # 4. Budget vs Actuals
    budget_vs_actual = []
    exp_by_cat = {}
    for e in list(db.scalars(select(Expense).where(Expense.tenant_id == tenant_id)).all()):
        cat = e.category or "Other"
        exp_by_cat[cat] = exp_by_cat.get(cat, 0.0) + float(e.amount or 0)
    for cat, actual_val in exp_by_cat.items():
        budget_vs_actual.append({
            "name": cat,
            "budget": actual_val * 1.15,
            "actual": actual_val
        })
