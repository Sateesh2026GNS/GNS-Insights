...
def get_batch_summary(db: Session, tenant_id: int) -> BatchSummaryRead:
    batches = list(db.scalars(select(Batch).where(Batch.tenant_id == tenant_id)).all())
    counts = {"running": 0, "completed": 0, "hold": 0, "rejected": 0, "expired": 0}
    for b in batches:
        s = (b.status or "").lower()
        if s in ("running", "in_progress", "active"):
            counts["running"] += 1
        elif s in ("completed", "closed", "done"):
            counts["completed"] += 1
        elif s in ("hold", "paused", "suspended"):
            counts["hold"] += 1
        elif s in ("rejected", "failed", "scrap"):
            counts["rejected"] += 1
        elif s in ("expired", "expired_hold"):
            counts["expired"] += 1
    return BatchSummaryRead(
        total_batches=len(batches),
