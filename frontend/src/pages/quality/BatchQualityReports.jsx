...
import { useToast } from "../../context/ToastContext";
import { getBatchEnriched, getBatchSummary } from "../../api/qualityApi";
import { formatPct } from "../../data/qualityMasterData";
...
export default function BatchQualityReports() {
  const { addToast } = useToast();
  const [loading, setLoading] = useState(true);
  const [summary, setSummary] = useState(INITIAL_BATCH_SUMMARY);
  const [rows, setRows] = useState([]);
  const [search, setSearch] = useState("");
  const [resultFilter, setResultFilter] = useState("");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSummary(INITIAL_BATCH_SUMMARY);
      setRows([]);
      addToast("Failed to load batch report data", "error");
    } finally {
      setLoading(false);
    }
  }, [addToast]);
  ...
  const chartData = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      const monthIndex = new Date(r.report_date).getMonth();
      const mName = months[monthIndex];
      if (!groups[mName]) {
        groups[mName] = { yieldSum: 0, failSum: 0, count: 0 };
      }
      groups[mName].yieldSum += r.yield_pct || 0;
      groups[mName].failSum += r.reject_qty || 0;
      groups[mName].count += 1;
    });

    const activeMonths = months.filter((m) => groups[m]);
    if (activeMonths.length === 0) return [];
    return activeMonths.map((m) => ({
      month: m,
      yield: Math.round((groups[m].yieldSum / groups[m].count) * 10) / 10,
      failures: groups[m].failSum,
    }));
  }, [rows]);
