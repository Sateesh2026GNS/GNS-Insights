...
=======
import { useCallback, useEffect, useMemo, useState } from "react";
import { AlertTriangle, ArrowRight, CheckCircle, ClipboardList, RefreshCw, User } from "lucide-react";
...
import { useToast } from "../../context/ToastContext";
import { getDefectsEnriched, getDefectSummary, updateDefectStatus } from "../../api/qualityApi";
import { DEFECT_WORKFLOW, qcStatusColor, severityColor } from "../../data/qualityMasterData";
...
export default function DefectTracking() {
  const { addToast } = useToast();
  const [loading, setLoading] = useState(true);
  const [summary, setSummary] = useState(INITIAL_DEFECT_SUMMARY);
  const [rows, setRows] = useState([]);
  const [search, setSearch] = useState("");
  const [resultFilter, setResultFilter] = useState("");
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSummary(INITIAL_DEFECT_SUMMARY);
      setRows([]);
      addToast("Failed to load defect tracking data", "error");
    } finally {
      setLoading(false);
    }
  }, [addToast]);
