...
export default function SettingsBankDetails() {
  ...
  const [sort, setSort] = useState({ key: "ac_name", dir: "asc" });
  ...
  const SortHeader = ({ colKey, label }) => (
    <div className="flex flex-col gap-1">
      <button type="button" onClick={() => setSort((s) => ({ key: colKey, dir: s.key === colKey && s.dir === "asc" ? "desc" : "asc" }))} className="flex items-center gap-1 text-left font-semibold text-slate-700 dark:text-slate-300">
        {label}
        <span className="flex">
      ...
      <input type="text" placeholder="Q Search" value={search[colKey] ?? ""} onChange={(e) => setSearch((s) => ({ ...s, [colKey]: e.target.value }))} className="rounded border border-slate-200 bg-white px-2 py-1 text-xs placeholder-slate-400 dark:border-slate-600 dark:bg-slate-800 dark:text-slate-200" />
    </div>
  );
    ...
    <div className="mx-auto max-w-6xl">
      <div className="mb-6 flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-500 dark:text-slate-400">This is a list of your bank details that will be used for providing payment details in documents</p>
        </div>
          ...
          <table className="w-full min-w-[800px] border-collapse">
            ...
            <tbody>
              {paginated.length === 0 ? (
                ...
                paginated.map((row) => (
                  <tr key={row.id} className="border-b border-slate-100 hover:bg-slate-50/50 dark:border-slate-700/50 dark:hover:bg-slate-800/30">
                    {COLUMNS.map((c) => (
                      <td key={c.key} className="px-4 py-3 text-sm text-slate-700 dark:text-slate-300">{row[c.key] ?? "—"}</td>
                    ))}
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm text-slate-600 dark:text-slate-400">{total === 0 ? "0 to 0 of 0" : `${start + 1} to ${Math.min(start + rowsPerPage, total)} of ${total}`}</span>
