...
import { Calendar, CheckCircle, Clock, Plus, RefreshCw, XCircle, X, Save } from "lucide-react";
...
export default function Leave() {
  ...
  const load = useCallback(async (isManual = false) => {
    ...
    try {
      ...
      if (isManual) {
        if (hasError) addToast("Failed to refresh leave data", "error");
        else addToast("Leave data refreshed", "success");
      }
    } catch {
      if (isManual) addToast("Failed to refresh leave data", "error");
    } finally {
      setLoading(false);
    }
  }, [addToast]);
