...
const INITIAL_INVENTORY = {
  kpis: [
    { key: "turnover", label: "Turnover Rate", value: 0, change_pct: null, unit: "x", format: "number" },
    { key: "outflow", label: "Outflow", value: 0, change_pct: null, unit: "units", format: "number" },
    { key: "avg_inv", label: "Average Inventory", value: 0, change_pct: null, unit: "units", format: "number" },
    { key: "value", label: "Inventory Value", value: 0, change_pct: null, format: "currency" },
    { key: "fast", label: "Fast Moving Items", value: 0, change_pct: null, format: "number" },
    { key: "slow", label: "Slow Moving Items", value: 0, change_pct: null, format: "number" },
    { key: "dead", label: "Dead Stock", value: 0, change_pct: null, format: "number" },
    { key: "reorder", label: "Reorder Alerts", value: 0, change_pct: null, format: "number" },
    { key: "accuracy", label: "Stock Accuracy", value: 0, change_pct: null, unit: "%", format: "percent" },
    { key: "warehouse", label: "Warehouse Utilization", value: 0, change_pct: null, unit: "%", format: "percent" },
  ],
  alerts: [],
  stock_in_vs_out: [],
  warehouse_occupancy: [],
...
export default function InventoryAnalytics() {
  const { addToast } = useToast();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState(INITIAL_INVENTORY);
  const [autoRefresh, setAutoRefresh] = useState(false);
  const [filters, setFilters] = useState({
    fiscalYear: "2025-26", month: "All Months", quarter: "All Quarters",
    plant: "All Plants", department: "All Departments", warehouse: "All Warehouses",
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      addToast("Failed to load inventory analytics data", "error");
    } finally {
      setLoading(false);
    }
  }, [addToast]);
