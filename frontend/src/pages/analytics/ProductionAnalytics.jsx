...
const INITIAL_PRODUCTION = {
  kpis: [
    { key: "planned", label: "Planned Production", value: 0, change_pct: null, unit: "units", format: "number", drill_target: "monthly" },
    { key: "actual", label: "Actual Production", value: 0, change_pct: null, unit: "units", format: "number", drill_target: "monthly" },
    { key: "efficiency", label: "Production Efficiency", value: 0, change_pct: null, unit: "%", format: "percent", drill_target: "machine" },
    { key: "oee", label: "OEE", value: 0, change_pct: null, unit: "%", format: "percent", drill_target: "machine" },
    { key: "utilization", label: "Machine Utilization", value: 0, change_pct: null, unit: "%", format: "percent", drill_target: "machine" },
    { key: "rejection", label: "Rejection %", value: 0, change_pct: null, unit: "%", format: "percent", drill_target: "quality" },
    { key: "downtime", label: "Downtime Hours", value: 0, change_pct: null, unit: "h", format: "number", drill_target: "downtime" },
    { key: "cost", label: "Production Cost", value: 0, change_pct: null, format: "currency", drill_target: "cost" },
    { key: "wip", label: "WIP", value: 0, change_pct: null, unit: "units", format: "number", drill_target: "wip" },
    { key: "completed", label: "Completed Orders", value: 0, change_pct: null, format: "number", drill_target: "orders" },
    { key: "worker", label: "Worker Performance", value: 0, change_pct: null, unit: "%", format: "percent", drill_target: "operator" },
    { key: "avg_month", label: "Avg / Month", value: 0, change_pct: null, unit: "units", format: "number", drill_target: "monthly" },
  ],
  alerts: [],
  benchmarks: [],
  monthly_production: [],
...
export default function ProductionAnalytics() {
  const { addToast } = useToast();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState(INITIAL_PRODUCTION);
  const [autoRefresh, setAutoRefresh] = useState(false);
  const [activeKpi, setActiveKpi] = useState(null);
  const [filters, setFilters] = useState({
    fiscalYear: "2025-26", month: "All Months", quarter: "All Quarters",
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(INITIAL_PRODUCTION);
      addToast("Failed to load production analytics data", "error");
    } finally {
      setLoading(false);
    }
  }, [addToast]);
