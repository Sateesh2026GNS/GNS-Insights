...
function normalizeUser(raw) {
  ...
  // Check if there is a persisted active role in localStorage
  let activeRole = raw.role ?? "Operator";
  try {
    const persistedActiveRole = localStorage.getItem("smrt-active-role");
    if (persistedActiveRole && roles.includes(persistedActiveRole)) {
...
export function AuthProvider({ children }) {
  ...
  const login = (authData) => {
    ...
    if (typeof authData === "object" && authData !== null) {
      ...
      if (u && u.role) {
        try {
          localStorage.setItem("smrt-active-role", u.role);
        } catch {}
      }
      if (token) {
        try {
  ...
  const logout = async () => {
    ...
    try {
      ...
      localStorage.removeItem("smrt-token");
      localStorage.removeItem("smrt-refresh-token");
      localStorage.removeItem("smrt-active-role");
    } catch {}
  };
