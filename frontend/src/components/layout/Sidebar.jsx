...
export default function Sidebar({ collapsed, onClose }) {
  ...
  const isAccountant = useMemo(() => {
    ...
    return roles.some((role) => String(role).toLowerCase() === "accountant");
  ...
  const visibleNav = useMemo(() => {
    return SIDEBAR_NAV.map((section) => {
      ...
      const children = (section.children || []).filter((c) => {
        if (!userCanAccess(user, c.module)) return false;
        if (!isAccountant && (c.to === "/vendors" || c.to === "/inventory/suppliers")) {
          return false;
        }
        return true;
      });
